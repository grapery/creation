...
export const tags = {
    ...
    getPopular: async (limit = 20): Promise<{ tags: Tag[]; total: number }> => {
        return request(`/api/v1/tags/popular?limit=${limit}`);
    },

    // Get stories by tag
    getStoriesByTag: async (tagId: string, page = 1, limit = 20): Promise<{ stories: unknown[]; total: number }> => {
        const offset = (page - 1) * limit;
        return request(`/api/v1/tags/${tagId}/stories?limit=${limit}&offset=${offset}`);
    },

    // Get story tags
    getStoryTags: async (storyId: string): Promise<{ tags: Tag[] }> => {
        return request(`/api/v1/stories/${storyId}/tags`);
    },

    // Add tags to story
    addToStory: async (storyId: string, tagNames: string[]): Promise<void> => {
        return request(`/api/v1/stories/${storyId}/tags`, 'POST', { tags: tagNames });
    }
};
