...
export async function GET(request: NextRequest) {
    try {
        ...
        const backendUrl = `${BACKEND_URL}/api/v1/storyboards/feed${queryString ? `?${queryString}` : ''}`;
        ...
        if (!response.ok) {
            let errorMessage = 'Failed to fetch storyboard feed';
            try {
        ...
        console.error('[API /storyboards/feed] Exception caught:', error);
