...
export async function GET(
    ...
    try {
        ...
        const backendUrl = `${BACKEND_URL}/api/v1/users/${userId}/storyboards${queryString ? `?${queryString}` : ''}`;
        ...
        if (!response.ok) {
            let errorMessage = 'Failed to fetch storyboards';
            try {
        ...
        console.error('[API /users/[id]/storyboards] Exception caught:', error);
