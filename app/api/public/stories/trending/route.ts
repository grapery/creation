...
export async function GET(request: NextRequest) {
    try {
        ...
        const backendUrl = `${BACKEND_URL}/api/public/stories/trending${queryString ? `?${queryString}` : ''}`;
        ...
        if (!response.ok) {
            let errorMessage = 'Failed to fetch trending stories';
            try {
        ...
        console.error('[API /public/stories/trending] Exception caught:', error);
