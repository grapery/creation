...
export async function POST(
    ...
    try {
        ...
        const backendUrl = `${BACKEND_URL}/api/groups/${groupId}/follow`;
        ...
        if (!response.ok) {
            let errorMessage = 'Failed to follow group';
            try {
        ...
        console.error('[API /groups/[id]/follow] Exception caught:', error);
...
export async function DELETE(
    ...
    try {
        ...
        if (!response.ok) {
            let errorMessage = 'Failed to unfollow group';
            try {
                const errorData = await response.json();
                errorMessage = errorData.message || errorData.msg || errorMessage;
            } catch (e) {}
