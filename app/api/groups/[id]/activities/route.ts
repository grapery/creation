...
export async function GET(
    ...
    try {
        ...
        const backendUrl = `${BACKEND_URL}/api/groups/${groupId}/activities${queryString ? `?${queryString}` : ''}`;
        ...
        if (!response.ok) {
            let errorMessage = 'Failed to fetch group activities';
            try {
        ...
        console.error('[API /groups/[id]/activities] Exception caught:', error);
