...
export async function GET(request: NextRequest) {
    try {
        ...
        const backendUrl = `${BACKEND_URL}/api/groups/search${queryString ? `?${queryString}` : ''}`;
        ...
        if (!response.ok) {
            let errorMessage = 'Failed to search groups';
            try {
        ...
        console.error('[API /groups/search] Exception caught:', error);
