...
export async function GET(request: NextRequest) {
    try {
        ...
        const backendUrl = `${BACKEND_URL}/api/groups/invites${queryString ? `?${queryString}` : ''}`;
        ...
        if (!response.ok) {
            let errorMessage = 'Failed to fetch group invites';
            try {
        ...
        console.error('[API /groups/invites] Exception caught:', error);
