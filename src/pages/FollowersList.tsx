...
import { mockFollowersList } from '../lib/mockData';
...
export function FollowersList({ onNavigate }: FollowersListProps) {
  ...
  const totalPages = Math.ceil(mockFollowersList.length / itemsPerPage);
  const startIndex = (currentPage - 1) * itemsPerPage;
  const endIndex = startIndex + itemsPerPage;
  const displayedFollowers = mockFollowersList.slice(startIndex, endIndex);
